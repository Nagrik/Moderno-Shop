...
export const CartPage = () => {
    ...
    const addedClothesPrice = Object.keys(items).map(key => {
        return items[key].totalPrice
    })

    console.log(addedClothesPrice[addedClothesPrice.length-1])
    console.log(items)
    return (
                    ...
                    <div className="content__items">
                        {
                            addedClothes.map((obj,index) =>
                                //@ts-ignore
                                <CartItem name={obj.items.name} type={obj.items.category} key={index} totalPrice={items[obj.items.id].totalPrice} />)
                        }
                    </div>
                    <div className="cart__bottom">
                        <div className="cart__bottom-details">
                            <span> Total clothes: <b>{totalCount}</b> </span>
